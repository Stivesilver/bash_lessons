<?php
	...
	if (in_array(IDEAFormat::get('id'), array(19, 29))) { 

		$edit->addGroup('Other Information');

		$edit->addControl("Post-secondary Transition", "select_check")
			->sqlField('mo_domains')
			->sql("
                SELECT validvalueid,
                       validvalue
                  FROM webset.glb_validvalues
                 WHERE valuename = 'MO_BGB_Domains'
                 ORDER BY validvalueid , validvalue
			 ")
			 ->displaySelectAllButton(FALSE);

		$edit->addControl("Progress measured by", "select_check")
			->name('meas_id')
			->value(db::execSQL("
                SELECT  plpgsql_recs_to_str('
                            SELECT cast(meas_refid as varchar) AS column
                              FROM webset.std_bgb_goal_meas
                             WHERE goal_refid = " . $RefID . "' , ',') as meas_id
                    ")->getOne())
			->sql("
		...
		$edit->addControl('Other Progress Measure')
			->name('meas_other')
			->value(
				db::execSQL("
					SELECT other
					  FROM webset.std_bgb_goal_meas std
					       INNER JOIN webset.glb_validvalues glb ON std.meas_refid = glb.refid
					 WHERE goal_refid = " . $RefID . "
					   AND validvalue ILIKE 'Other%'
                    ")->getOne()
			)
			->showIf('meas_id', $other_def_id)
			->css('width', '100%');

		$edit->addControl('Comments', 'textarea')
			->sqlField('mo_comments')
			->css('width', '100%');
	} else {
		$edit->addControl("Direction of " . $titles['goal'], "select")
			->sqlField('gdirection')
			->sql("
                SELECT dgrefid,
                       dgtext
                  FROM webset.statedef_dirofgoal
