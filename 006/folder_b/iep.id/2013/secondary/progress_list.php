<?php
	...
	$progress = db::execSQL("
        SELECT refid as sprrefid,
         ...
         WHERE stdrefid = " . $tsRefID . "
           AND area_id = " . IDEAAppArea::ID_SEC_PROGRESS . "
         ORDER BY 2, 3, 5, 6
    ")->assocAll();
	...
	$list = new ListClass();
	$list->title = 'Secondary IEP Goals Progress Report';
	$list->hideCheckBoxes = true;
	$list->hideNumberColumn = true;

	$list->SQL = "
       SELECT * FROM (SELECT refid as grefid,
                             txt03 AS gsentance,
                             NULL AS bsentance,
                             NULL AS orefid,
                             order_num AS g_num,
                       ...
                       WHERE stdrefid = " . $tsRefID . "
                         ...
                         AND area_id = " . IDEAAppArea::ID_SEC_GOALS . "
                       UNION ALL
                      SELECT g.refid as grefid,
                        ...
                        FROM webset.std_general g
			                 INNER JOIN webset.disdef_bgb_goaldomainscopeksa ksa ON int01 = ksa.gdskrefid
			                 INNER JOIN webset.std_general o ON g.refid = o.int01 AND o.area_id = " . IDEAAppArea::ID_SEC_OBJECTIVES . "
                       WHERE g.stdrefid = " . $tsRefID . "
                         AND g.iepyear = " . $siymrefid . "
                         AND g.area_id = " . IDEAAppArea::ID_SEC_GOALS . "
                     ) as t
