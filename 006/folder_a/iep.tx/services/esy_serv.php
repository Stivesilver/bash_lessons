<?php
	...
	$form_state_id = 870;
	$form_std_id = (int) IDEAStudentRegistry::readStdKey($tsRefID, 'tx_iep', 'ESY_Services_form', $stdIEPYear);
	...
	$edit->title = 'Extended School Year Services';
	...
	$edit->addControl(
		FFSwitchYN::factory('ESY Services were recommended')
			->name('esy')
			->value(IDEAStudentRegistry::readStdKey($tsRefID, 'tx_iep', 'ESY Services_chk', $stdIEPYear))
	);

	$edit->addControl('ESY Form', 'protected')
		->showIf('esy', 'Y')
		->name('transport_form')
		->append(
	...
	$edit->finishURL = CoreUtils::getURL('esy_serv_save.php', array('dskey' => $dskey));
	$edit->saveURL = CoreUtils::getURL('esy_serv_save.php', array('dskey' => $dskey));
	...
	function formUrlPrepare($state_id, $std_id, $dskey, $tsRefID) {
		...
		return IDEAForm::factory()
				...
				->setUrlSave(CoreUtils::getPhysicalPath('esy_serv_form_save.php'))
				->setUrlFinish(CoreUtils::getURL('esy_serv_form_save.php'))
