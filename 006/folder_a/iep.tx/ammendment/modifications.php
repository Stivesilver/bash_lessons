<?php
	...
	$values     = IDEADef::getValidValues('TX_IEP_Amendments_Mods');
	$checkboxes = array();
	...
	foreach ($values as $val) {
		$key              = $val->get(IDEADefValidValue::F_VALUE_ID);
		$checkboxes[$key] = '<i>' . $val->get(IDEADefValidValue::F_VALUE) . '</i>';
	}
	...
	$edit->addControl('Address and include only the applicable attachments', 'select_check')
		...
		->breakRow()
		->name('selected')
		->value(IDEAStudentRegistry::readStdKey($tsRefID, 'tx_amm', 'modifications', $stdIEPYear));
