<?php
    ...
    $list->addSearchField('Mode', 'blankornot', 'list')
        ->data(array(1=>'Blank Medicaid ID #', 2=>'None Blank Medicaid ID #'))
        ->sqlField("CASE WHEN COALESCE(stdmedicatenum,'') = '' THEN 1 ELSE 2 END");

    $list->addColumn('Student Name');
    $list->addColumn('Reporting School');
    $list->addColumn('Grade');
    $list->addColumn('Medicaid ID #');
