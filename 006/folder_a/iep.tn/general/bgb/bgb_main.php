<?php
	...
	$list2->addColumn($titles['goal'], '45%')->dataCallback('markCurGoal');
	$list2->addColumn('Timeline', '45%')->sqlField('gdate')->type('date');
	...
	$list3->SQL = "
        SELECT brefid,
               COALESCE(overridetext, bsentance) AS act_step,
               in_support,
               order_num
	...
	$list3->addColumn('Order', '5%')->sqlField('order_num');
	$list3->addColumn($titles['benchmark'])->sqlField('act_step');
	$list3->addColumn('Person(s) Responsible')->sqlField('in_support');
