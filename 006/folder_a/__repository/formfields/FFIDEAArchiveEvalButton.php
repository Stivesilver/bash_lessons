<?php
	...
	/**
	 * Creates button for Archiving
	 *
	 * @copyright Lumen Touch, 2015
	 */
	class FFIDEAArchiveEvalButton extends FFButton {
		...
		public function checkAccess() {
			if(SystemCore::$isAdmin == false) {
				...
				$is_pc = $this->execSQL("
					...
					")->getOne();
				if ($is_pc) {
					if (IDEACore::disParam(157) == 'N') {
						$this->archAccess = false;
					} else {
						$this->archAccess = true;
					}
				} 
				# If user is not Lumen Admin and 158 param is ON - NO ACCESS
				# This param overwrites previous ones
				if (IDEACore::disParam(158) == 'Y') {
					$this->archAccess = false;
				}
			}
		}
		 ...
		 * @throws Exception
		 * @return FFIDEAArchiveEvalButton
		 */
		...
		public static function factory($caption = 'Archive') {
			return new FFIDEAArchiveEvalButton($caption);
		}
