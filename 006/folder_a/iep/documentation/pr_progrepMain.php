<?php
    ...
    $iepyears = FFSelect::factory('IEP Year')
        ...
        ")
        ->onChange("api.goto(api.url('pr_progrepMain.php', {'dskey' : '" . $dskey . "', 'ESY' : '" . $esy . "', 'siymrefid' : this.value}))");
    ...
    $print_button = FFButton::factory('Print')
        ...
        ->onClick('api.ajax.process(ProcessType.REPORT, "' . CoreUtils::getURL('pr_progrep_print.ajax.php', array('dskey' => $dskey, 'tsRefID' => $tsRefID, 'ESY' => $esy, 'siymrefid' => $siymrefid)) . '")');
    ...
    print FormField::factory('hidden')->name('esy')->value($esy)->toHTML();
    ...
    function showProgressMark($data, $col) {
        ...
        return FFButton::factory('(add)', 'addProgress(' . $periods[$col]['bmnum'] . ', ' . $data['grefid'] . ', ' . (int) $data['brefid'] . ', ' . $periods[$col]['dsyrefid'] . ')')
                ->width('50%')
