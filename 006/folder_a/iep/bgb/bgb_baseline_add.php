<?php
	...
	$edit->title = 'Add/Edit ' . $titles['baseline'];
	...
	$edit->addControl("Narrative", "textarea")->sqlField('blbaseline')->css("width", "100%")->req(IDEACore::disParam(36) == "N" ? false : true);
	...
	io::js('
		...
		edit1.onSaveDoneFunc(
			function (refid) {
				if (recordAdded) {
					api.confirm("Do you want to add a ' . $titles['goal'] . ' for this ' . $titles['baseline'] . '?", onOk, onNo);
					return false;
				}
			}
		)
